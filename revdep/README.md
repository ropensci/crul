# Setup

## Platform

|setting  |value                                       |
|:--------|:-------------------------------------------|
|version  |R version 3.4.3 Patched (2018-01-01 r74017) |
|system   |x86_64, darwin15.6.0                        |
|ui       |X11                                         |
|language |(EN)                                        |
|collate  |en_US.UTF-8                                 |
|tz       |America/Los_Angeles                         |
|date     |2018-01-19                                  |

## Packages

|package |*  |version |date       |source                   |
|:-------|:--|:-------|:----------|:------------------------|
|crul    |   |0.5.0   |2018-01-19 |local (ropensci/crul@NA) |

# Check results

41 packages

|package          |version | errors| warnings| notes|
|:----------------|:-------|------:|--------:|-----:|
|bold             |0.5.0   |      0|        0|     0|
|ccafs            |0.1.0   |      0|        0|     0|
|crminer          |0.1.4   |      0|        0|     0|
|discgolf         |0.2.0   |      0|        0|     0|
|duckduckr        |1.0.0   |      0|        0|     0|
|enigma           |0.3.0   |      0|        0|     0|
|fauxpas          |0.1.0   |      0|        0|     0|
|fulltext         |1.0.0   |      0|        0|     0|
|jaod             |0.1.0   |      0|        0|     0|
|microdemic       |0.2.0   |      0|        0|     0|
|natserv          |0.1.4   |      0|        0|     0|
|nneo             |0.1.0   |      0|        0|     0|
|openadds         |0.2.0   |      0|        0|     0|
|pangaear         |0.6.0   |      0|        0|     0|
|pleiades         |0.2.0   |      0|        0|     0|
|postlightmercury |1.2     |      0|        0|     0|
|rbhl             |0.8.0   |      0|        0|     0|
|rbison           |0.5.4   |      0|        0|     0|
|rcoreoa          |0.1.0   |      0|        0|     0|
|rcrossref        |0.8.0   |      0|        0|     0|
|rdatacite        |0.3.0   |      0|        0|     0|
|rdpla            |0.2.0   |      0|        0|     0|
|rdryad           |0.3.0   |      0|        0|     0|
|rgbif            |0.9.9   |      0|        0|     0|
|rif              |0.2.0   |      0|        0|     0|
|ritis            |0.7.0   |      0|        0|     0|
|rjsonapi         |0.1.0   |      0|        0|     0|
|rnoaa            |0.7.0   |      0|        0|     0|
|ropenaq          |0.2.5   |      0|        0|     0|
|rplos            |0.8.0   |      0|        0|     0|
|rredlist         |0.4.0   |      0|        0|     0|
|rtimes           |0.5.0   |      0|        0|     0|
|rvertnet         |0.6.2   |      0|        0|     0|
|seaaroundus      |1.2.0   |      0|        0|     0|
|sofa             |0.3.0   |      0|        0|     0|
|solrium          |1.0.0   |      0|        0|     0|
|spocc            |0.7.0   |      0|        0|     0|
|traits           |0.3.0   |      0|        0|     0|
|webmockr         |0.1.0   |      0|        0|     0|
|wikitaxa         |0.2.0   |      0|        0|     0|
|worrms           |0.2.0   |      0|        0|     0|

## bold (0.5.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/bold/issues

0 errors | 0 warnings | 0 notes

## ccafs (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/ccafs/issues

0 errors | 0 warnings | 0 notes

## crminer (0.1.4)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropensci/crminer/issues

0 errors | 0 warnings | 0 notes

## discgolf (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/sckott/discgolf/issues

0 errors | 0 warnings | 0 notes

## duckduckr (1.0.0)
Maintainer: Schumacher Dirk <mail@dirk-schumacher.net>  
Bug reports: https://github.com/dirkschumacher/duckduckr/issues

0 errors | 0 warnings | 0 notes

## enigma (0.3.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropengov/enigma/issues

0 errors | 0 warnings | 0 notes

## fauxpas (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropenscilabs/fauxpas/issues

0 errors | 0 warnings | 0 notes

## fulltext (1.0.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/fulltext/issues

0 errors | 0 warnings | 0 notes

## jaod (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropenscilabs/jaod/issues

0 errors | 0 warnings | 0 notes

## microdemic (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropenscilabs/microdemic/issues

0 errors | 0 warnings | 0 notes

## natserv (0.1.4)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/natserv/issues

0 errors | 0 warnings | 0 notes

## nneo (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropenscilabs/nneo/issues

0 errors | 0 warnings | 0 notes

## openadds (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/sckott/openadds/issues

0 errors | 0 warnings | 0 notes

## pangaear (0.6.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/pangaear/issues

0 errors | 0 warnings | 0 notes

## pleiades (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/pleiades/issues

0 errors | 0 warnings | 0 notes

## postlightmercury (1.2)
Maintainer: Mikkel Freltoft Krogsholm <mikkel@56n.dk>

0 errors | 0 warnings | 0 notes

## rbhl (0.8.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rbhl/issues

0 errors | 0 warnings | 0 notes

## rbison (0.5.4)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rbison/issues

0 errors | 0 warnings | 0 notes

## rcoreoa (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropensci/rcoreoa/issues

0 errors | 0 warnings | 0 notes

## rcrossref (0.8.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rcrossref/issues

0 errors | 0 warnings | 0 notes

## rdatacite (0.3.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rdatacite/issues

0 errors | 0 warnings | 0 notes

## rdpla (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rdpla/issues

0 errors | 0 warnings | 0 notes

## rdryad (0.3.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rdryad/issues

0 errors | 0 warnings | 0 notes

## rgbif (0.9.9)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rgbif/issues

0 errors | 0 warnings | 0 notes

## rif (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropensci/rif/issues

0 errors | 0 warnings | 0 notes

## ritis (0.7.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/ritis/issues

0 errors | 0 warnings | 0 notes

## rjsonapi (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rjsonapi/issues

0 errors | 0 warnings | 0 notes

## rnoaa (0.7.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rnoaa/issues

0 errors | 0 warnings | 0 notes

## ropenaq (0.2.5)
Maintainer: Maëlle Salmon <maelle.salmon@yahoo.se>  
Bug reports: http://github.com/ropensci/ropenaq/issues

0 errors | 0 warnings | 0 notes

## rplos (0.8.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rplos/issues

0 errors | 0 warnings | 0 notes

## rredlist (0.4.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rredlist/issues

0 errors | 0 warnings | 0 notes

## rtimes (0.5.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropengov/rtimes/issues

0 errors | 0 warnings | 0 notes

## rvertnet (0.6.2)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/rvertnet/issues

0 errors | 0 warnings | 0 notes

## seaaroundus (1.2.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>

0 errors | 0 warnings | 0 notes

## sofa (0.3.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/sofa/issues

0 errors | 0 warnings | 0 notes

## solrium (1.0.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/solrium/issues

0 errors | 0 warnings | 0 notes

## spocc (0.7.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/spocc/issues

0 errors | 0 warnings | 0 notes

## traits (0.3.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/traits/issues

0 errors | 0 warnings | 0 notes

## webmockr (0.1.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropensci/webmockr/issues

0 errors | 0 warnings | 0 notes

## wikitaxa (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus+r@gmail.com>  
Bug reports: https://github.com/ropensci/wikitaxa/issues

0 errors | 0 warnings | 0 notes

## worrms (0.2.0)
Maintainer: Scott Chamberlain <myrmecocystus@gmail.com>  
Bug reports: https://github.com/ropensci/worrms/issues

0 errors | 0 warnings | 0 notes

